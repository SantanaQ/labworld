...
export const EditorSidebar: React.FC<EditorSidebarProps> = ({ onAddNode }) => {
    ...
    return (
        <div className={`h-full bg-zinc-900 border-r border-zinc-500 flex flex-col shrink-0 shadow-xl transition-all duration-300 ${collapsed ? 'w-14' : 'w-55'}`}>
            ...
            {!collapsed && (
                <div className="flex-1 overflow-y-auto overflow-x-hidden">
                    {Object.entries(categories).map(([catTitle, items]) => (
                        <div key={catTitle} className="border-b border-zinc-800/50">
                            <button
                                onClick={() => toggleCat(catTitle)}
                                className="w-full flex items-center justify-between p-3 hover:bg-zinc-800 transition-colors gap-2"
                            >
                                {/* truncate sorgt für ... wenn der Titel zu lang ist */}
                                <span className="text-sm font-medium text-zinc-300 truncate whitespace-nowrap">
                                {catTitle}
                            </span>
                            ...
                            {openCats.includes(catTitle) && (
                                <div className="bg-zinc-950/30 py-1">
                                    {items.map(item => (
                                        <div key={item.type} className="relative group/item">
                                            <button
                                                title={item.label}
                                                onClick={() => onAddNode(item.type)}
                                                className="w-full flex items-center gap-3 px-6 py-2 text-sm text-zinc-400 hover:text-white hover:bg-blue-600/20 transition-all overflow-hidden"
                                            >
                                                <Plus size={14}
                                            ...
                                            <div
                                                className="absolute left-full ml-2 px-2 py-1 bg-zinc-700 text-white text-[10px] rounded opacity-0 group-hover/item:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-xl border border-zinc-600 before:content-[''] before:absolute before:right-full before:top-1/2 before:-tranzinc-y-1/2 before:border-4 before:border-transparent before:border-r-zinc-700">
                                                Add {item.label}
                                            </div>
                                        </div>
                                    ))}
            ...
            {/* Footer */}
            {!collapsed && (
                <div className="text-zinc-600 italic text-[10px] p-2 shrink-0">
